...
from NeuStereo import config

class NeuStereo(torch.nn.Module):
    def __init__(self):
        super(NeuFlow, self).__init__()

        self.backbone = backbone.CNNEncoder(config.feature_dim_s16, config.context_dim_s16, config.feature_dim_s8, config.context_dim_s8)
        ...
        self.conv_s8 = backbone.ConvBlock(3, config.feature_dim_s1, kernel_size=8, stride=8, padding=0)
