...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None,
                 root='/media/hulk/1TB_data_drive/optical_flow/datasets/FlyingThings3D',
        ...
        for cam in cam_list:
            ...
            left_imgs = sorted([osp.join(f, 'left') for f in image_dirs]) #Reading Left Cam images
            right_imgs = sorted([osp.join(f, 'right') for f in image_dirs]) #Reading Right Cam images

            if test_set:
                flow_dirs = sorted(glob(osp.join(flow_dir, 'flyingthings3d__disparity/disparity/TEST/*/*')))
            else:
                flow_dirs = sorted(glob(osp.join(flow_dir, 'flyingthings3d__disparity/disparity/TRAIN/*/*')))
            #flow_dirs = sorted([osp.join(f, direction, cam) for f in flow_dirs])
            flow_dirs = sorted([osp.join(f, "left") for f in flow_dirs])
            
            for ldir, rdir, fdir in zip(left_imgs, right_imgs, flow_dirs):
                l_images = sorted(glob(osp.join(ldir, '*.npy')))
                r_images = sorted(glob(osp.join(rdir, '*.npy')))
...
def build_train_dataset(stage):
    if stage == 'chairs':
        ...
        clean_dataset = FlyingThings3D(aug_params, dstype='frames_cleanpass', only_left=True)
