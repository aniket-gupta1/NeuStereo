...
class NeuFlow(torch.nn.Module):
    ...
    def disparity_to_correspondence(self, disparity_up, grid=None):
        """
        Convert disparity map to correspondence map.
        
        Args:
            disparity: [B, 1, H, W] - ground truth disparity
            grid: [B, 2, H, W] - left image pixel coordinates (optional)
        
        Returns:
            correspondence: [B, 2, H, W] - where each left pixel maps to in right image
        """
        print(disparity_up.shape)

        # Downsample disparity 16 times
        disparity = F.avg_pool2d(disparity_up, kernel_size=16, stride=16)
        # Scale disparity values since resolution changed
        disparity = disparity / 16.0  # Important: scale the values!
        ...
        if grid is None:
            # Create meshgrid for left image coordinates
            y, x = torch.meshgrid(
                torch.arange(H, device=disparity.device, dtype=torch.float32),
                torch.arange(W, device=disparity.device, dtype=torch.float32),
                indexing='ij'
            )
        ...
        correspondence = grid.clone()
        
        # x_right = x_left - disparity
        correspondence[:, 0, :, :] = grid[:, 0, :, :] - disparity[:, 0, :, :]
        
        # y_right = y_left (no change for rectified stereo)
        correspondence[:, 1, :, :] = grid[:, 1, :, :]
        
        return correspondence  # [B, 2, H, W]

    def forward(self, img0, img1, iters_s16=1, iters_s8=8, disparity_gt=None):
        ...
        # correspondence_gt = self.disparity_to_correspondence(disparity_gt)
        flow0 = self.matching_s16.global_correlation_softmax(feature0_s16, feature1_s16, correspondence_gt=None)
        ...
        for i in range(iters_s8):
            ...
            if self.training or i == iters_s8 - 1:
                ...
                flow_list.append(up_flow0[:, 0, :, :].unsqueeze(1))
