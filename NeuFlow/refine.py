...
##Changes by Abhinav For extracting first 2 depth, for stereo
class ExtractFirstTwoChannels(torch.nn.Module):
    def __init__(self):
        super(ExtractFirstTwoChannels, self).__init__()

    def forward(self, x):
        # Extract the first 2 channels
        x = x[:, :2, :, :]
        # Set the second channel to 0
        x[:, 1] = 0
        return x
...
class Refine(torch.nn.Module):
    def __init__(self, context_dim, iter_context_dim, num_layers, levels, radius, inter_dim):
        ...
        self.extractor = ExtractFirstTwoChannels()
    ...
    def forward(self, corrs, context, iter_context, flow0):
        ...
        dflow = self.extractor(x) #using CNN to make row 1/y-axis as 0, to search only acrss the row

        return self.hidden_act(x[:,2:]), dflow#x[:,:2]
