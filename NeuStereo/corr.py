...
class StereoCorrBlock:
    ...
    def init_bhwd(self, B, H, W, device, amp):
        ...
        """
        #TODO: Maybe reduce the size to half() if using amp?
    ...
    def local_correlation_lookup(self, corr_volume, disp, radius):
        ...
        # 2.2) xL_base => row index in [0..W-1] #TODO: Can put both these in init
        # xL_base = torch.arange(W, device=corr_volume.device).view(1, W).float()  # => [1, W]
        # xL_base = xL_base.expand(B*H, -1)  # => [B*H, W]

        # 2.3) The offsets in [-radius..radius] #TODO: Can put both these in init
        # offset_vals = torch.arange(-radius, radius+1, device=corr_volume.device).view(1, 1, 2*radius+1).float() 
        # offset_vals = offset_vals.expand(B*H, W, -1)  # => [B*H, W, 2*r+1]
    ...
    def __call__(self, corr_pyramid, disp):
        ...
        for lvl, corr_lvl in enumerate(corr_pyramid):
            ...
            disp_l = F.interpolate(disp, size=(H_l, W_l), mode='bilinear', align_corners=False)
