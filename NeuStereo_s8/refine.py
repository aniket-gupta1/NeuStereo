...
class StereoRefine(torch.nn.Module):
    def __init__(self, context_dim, iter_context_dim, num_layers, levels, radius, inter_dim):
        super(StereoRefine, self).__init__()

        self.radius = radius
        # print("Conv1: ", (radius*2+1)**2*levels+context_dim+iter_context_dim+1+1, context_dim+iter_context_dim)
        # print("Conv2: ", context_dim+iter_context_dim, inter_dim)
        self.conv1 = ConvBlock((radius*2+1)*levels+context_dim+iter_context_dim+1+1, context_dim+iter_context_dim, kernel_size=3, stride=1, padding=1)
        self.conv2 = ConvBlock(context_dim+iter_context_dim, inter_dim, kernel_size=3, stride=1, padding=1)
