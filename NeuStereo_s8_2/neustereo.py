...
from NeuStereo_s8_2 import backbone
from NeuStereo_s8_2 import transformer
from NeuStereo_s8_2 import matching
from NeuStereo_s8_2 import corr
from NeuStereo_s8_2 import refine
from NeuStereo_s8_2 import upsample
import pdb
...
class NeuStereo(torch.nn.Module):
    def __init__(self, config):
        ...
        self.stereo_corr_block_s8 = corr.CorrBlock(radius=4, levels=1)
        ...
        self.stereo_refine_s8 = refine.Refine(self.config.context_dim_s8, self.config.iter_context_dim_s8, num_layers=5, levels=1, radius=4, inter_dim=96)
    ...
    def forward(self, img0, img1, iters_s16=1, iters_s8=8):
        ...
        features_context_s16, features_context_s8 = self.backbone(torch.cat([img0, img1], dim=0))

        # Compute cross attention to condition features on s16 features
        features_context_s16 = self.cross_attn_s16(features_context_s16)

        # Split into features and context
        features_s16, context_s16 = self.split_features(features_context_s16, self.config.context_dim_s16, self.config.feature_dim_s16)
        features_s8, context_s8 = self.split_features(features_context_s8, self.config.context_dim_s8, self.config.feature_dim_s8)
        
        # Upsample s16 features and merge with s8 features
        features_s16_upsampled = F.interpolate(features_s16, scale_factor=2, mode='nearest')
        features_s8 = self.merge_s8(torch.cat([features_s8, features_s16_upsampled], dim=1))
        ...
        # Merge context of s8 and s16 features
        context_s16_upsampled = F.interpolate(context_s16, scale_factor=2, mode='nearest')
        context_s8 = self.context_merge_s8(torch.cat([context_s8, context_s16_upsampled], dim=1))
